...
namespace ServiceModelEx.ServiceBus
{
    ...
    {
        public static string DefaultIssuer = "RootManageSharedAccessKey";
        ...
        static void SetServiceBusCredentials(IEnumerable<ServiceEndpoint> endpoints, string issuer, string secret)
        {
            ...
            //behavior.TokenProvider = TokenProvider.CreateSharedSecretTokenProvider(issuer, secret);
            behavior.TokenProvider = TokenProvider.CreateSharedAccessSignatureTokenProvider(issuer, secret);
        ...
        public static void SetServiceBusCredentialsWithSasPolicy(this ServiceHost host, string sasPolicyName, string sasPolicyKey)
        {
            if (host.State == CommunicationState.Opened)
            {
                throw new InvalidOperationException("Host is already opened");
            }
            SetServiceBusCredentials(host.Description.Endpoints, sasPolicyName, sasPolicyKey);
        }
